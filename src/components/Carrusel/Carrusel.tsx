...
const DateBadge = ({ data_index, variant, children, ...props }: DateBadgeProps) => {
    return (
        <button data-index={data_index} className={badgeVariants({ variant })} {...props}>
            {children}
        </button>
    )
}
        ...
        <Carousel
            ...
            <CarouselContent>
                ...
                <CarouselItem key={index} className="basis-1/8">
                    <DateBadge data_index={index} variant={index === (current - 1) ? "selected" : "unselected"} onClick={moveIndex}>
                        {data}
                    </DateBadge>
                </CarouselItem>
                )})}
