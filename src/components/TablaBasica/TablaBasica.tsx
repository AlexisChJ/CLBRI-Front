...
export function TablaBasica({
  ...
  return (
    <div className={`mb-10 ${className}`}>
      <h2 className={`${redhat_700.className} text-xl font-semibold text-[#5B5B5B]`}>
        {titulo}
      </h2>
          ...
          <TableRow className="border-none hover:bg-transparent">
            {encabezados.map((encabezado, index) => {
              ...
              return (
                <TableHead
                  key={index}
                  className={`${zenmaru_700.className} text-lg text-[#5B5B5B] border-none ${
          ...
          {datos.map((fila, filaIndex) => (
            ...
            >
              {encabezados.map((encabezado, colIndex) => {
                const esObjeto = typeof encabezado === 'object';
                const key = esObjeto ? encabezado.key || encabezado.label : encabezado;
                const align = esObjeto ? encabezado.align : undefined;
                const icono = esObjeto ? encabezado.icono : undefined;
                const valor = typeof fila === 'object' ? fila[key] : fila[colIndex];

                return (
                  <TableCell
                    key={colIndex}
                    className={`${zenmaru_700.className} text-md text-[#5B5B5B] border-none ${
                      align === 'right' ? 'text-right' :
                      align === 'center' ? 'text-center' : ''
                    }`}>
